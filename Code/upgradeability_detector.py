...
# --- API KEYS (keep your existing keys here if needed) ---
...
def build_solc_include_paths() -> list:
    """
    RETURNS ONLY VALID, ALWAYS-PRESENT PATHS
    This guarantees --include-path is never empty.
    """
    return [
        "/app",           # Your mounted directory
        "/app/src",       # If exists
        "/app/lib",       # If exists
        "/usr",           # Always exists
        "/usr/bin",       # Contains solc
        "/usr/lib"        # System libs
    ]
...
def slither_analyze(file_path: str) -> Dict[str, Any]:
    ...
    Uses a robust solc include-path/remapping strategy so the user does not
    have to reorganize the OpenZeppelin files.
    """
    results = {
        "name": file_path,
    ...
    try:
        # Build include paths and remappings
        include_paths = build_solc_include_paths()
        # Build solc_args string with allow-paths and include-paths
        # --allow-paths accepts comma separated directories
        allow_paths = ",".join(include_paths) if include_paths else "."
        solc_args_parts = [f"--allow-paths {allow_paths}"]
        # Add explicit include-path params for the compiler
        for p in include_paths:
            if os.path.isdir(p):
                solc_args_parts.append(f"--include-path {p}")

        solc_args = " ".join(solc_args_parts)

        # Helpful remappings to let imports like
        # "../lib/openzeppelin-contracts-upgradeable/contracts/..." resolve
        # even if files are flattened or in same directory
        solc_remaps = [
            # map common prefixes to the container /app or current working dir
            "lib/=./",
        ...
        # prefer the system solc binary installed in the image (slither expects this)
        solc_path = "/usr/bin/solc" if os.path.exists("/usr/bin/solc") else "solc"

        # Create Slither instance
        slither = Slither(file_path, solc=solc_path, solc_args=solc_args, solc_remaps=solc_remaps)
        ...
        # --- Detection Logic (2.2, 2.3, 2.4 are implemented below) ---
        ...
        for func in main_contract.functions:
            # Detect upgradeTo function signature or other upgrade functions
            if func.full_name.startswith('upgradeTo(') or 'upgradeTo' in func.name:
        ...
        for function in main_contract.functions_and_modifiers:
            # low_level_calls may contain DELEGATECALL etc.
        ...
        # Preserve the file name even on compilation failure
        results["name"] = file_path
        results["proxy_pattern"] = f"Solc Compile Fail"
        results["admin_control"] = f"Solidity Error: {str(e)[:120]}..."
        # Try to create a concise compilation message
        err_line = str(e).splitlines()[0] if isinstance(e, Exception) else str(e)
        results["staged_analysis"]["Compilation"] = f"Failed: {err_line[:150]}..."
        # Print error details for user visibility
...
def get_main_contract(slither_instance: Slither) -> Optional[Contract]:
    ...
    if not slither_instance.contracts:
        return None
    # heuristic: return the contract with the longest name (was used previously)
...
def print_deployed_status(results_list: List[Dict[str, Any]], console: Console):
    ...
    deployed_results = [r for r in results_list if r.get("Address", "N/A (Source)") != "N/A (Source)"]
    ...
    for result in deployed_results:
        pattern = result.get("Upgradeability Pattern", "Unknown")
...
def print_staged_results(results_list: List[Dict[str, Any]], console: Console):
    ...
    for result in results_list:
        # Only print staged analysis for source files (Address == N/A (Source))
        if result.get("Address", "N/A (Source)") == "N/A (Source)" and "staged_analysis" in result:
            analysis = result.get("staged_analysis", {})
            # Determine overall status of the staging process
            status_style = "bold green" if analysis.get("Compilation") == "Success" else "bold red"

            panel_content = Text()
            panel_content.append(f"Contract: {result.get('name', result.get('Contract', 'Unknown'))}\n", style="bold cyan")
            panel_content.append(f"Final Risk: {result.get('Risk Score', 'UNKNOWN')}\n\n", style="bold yellow")
            ...
            console.print(Panel(
                panel_content,
                title=Text(f"STAGED ANALYSIS: {analysis.get('Compilation', 'Unknown')}", style=status_style),
...
def print_results_cli(results_list: List[Dict[str, Any]], console: Console):
    ...
    for result in results_list:
        score = result.get("Risk Score", "UNKNOWN")
        ...
        contract_id = result.get("Address", "N/A (Source)") if result.get("Address", "N/A (Source)") != "N/A (Source)" else result.get("Contract", "Unknown")

        table.add_row(
            contract_id,
            result.get("Upgradeability Pattern", "Unknown"),
            result.get("Admin Control Type", "Unknown"),
            Text(str(score), style=score_style),
            result.get("Security Finding", "")
        )
...
def analyze_and_score(analysis_result: Dict[str, Any], contract_address: Optional[str] = None, console: Optional[Console] = None) -> Dict[str, Any]:
    ...
    # Guarantee a name exists (prevents KeyError downstream)
    if "name" not in analysis_result or not analysis_result.get("name"):
        if contract_address:
            analysis_result["name"] = f"Contract@{contract_address[:8]}..."
        else:
            analysis_result["name"] = analysis_result.get("Contract", "Unknown")

    proxy_info = proxy_detector(analysis_result, contract_address, console)
    detected_proxy = proxy_info.get("pattern", "Non-Upgradeable")
    admin_control = proxy_info.get("admin_control", analysis_result.get("admin_control", "None"))
    ...
    if detected_proxy == "Skipped (API Failure)":
        ...
        if has_delegatecall:
            # you might want to increase severity here or add warnings
            pass
    ...
    results = {
        "Contract": analysis_result.get("name", "Unknown"),
...
def main():
    ...
    for item in items_to_process:
        ...
        if is_file:
            ...
            try:
                # Provide a robust minimal analysis result for scoring and printing
                analysis_data = {
                    "name": f"Contract@{item[:8]}...",
                    "proxy_pattern": "Storage Check",
